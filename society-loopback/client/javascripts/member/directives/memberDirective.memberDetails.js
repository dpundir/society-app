...
define([
	...
	'javascripts/common/services/validation-service'
], function (angular, _) {
    angular.module("societyApp.member.directives.memberDetails", [
		'bootstrapLightbox',
		"societyApp.member.services.member",
		'societyApp.common.services.validationService'
	])
	.directive('memberDetails', ['fileUpload', 'SelectOptions', 'Lightbox', 'validationService', function (fileUpload, SelectOptions, Lightbox, validationService) {
		return {
			restrict: 'A',
			scope: {
				person: '=',
			...
			controller: ['$scope', function ($scope) {
				...
				function validateRegistrationForm(form) {
					if (form.$invalid) {
						var formFields = ['first_name', 'last_name', 'father_first_name', 'father_last_name', 'mother_first_name', 'mother_last_name', 'gender', 'maritalStatus', 'phone', 'dob', 'address1', 'address2', 'city', 'state', 'pincode','guardianType'];
				...
				$scope.phonePattern = validationService.getPhoneNumberPattern();
